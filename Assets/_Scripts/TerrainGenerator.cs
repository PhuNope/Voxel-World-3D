...
public class TerrainGenerator : MonoBehaviour {
    ...
    public ChunkData GenerateChunkData(ChunkData data, Vector2Int mapSeedOffset) {
        ...
        for (int x = 0; x < data.chunkSize; x++) {
            for (int z = 0; z < data.chunkSize; z++) {
                biomeSelection = SelectBiomeGenerator(new Vector3Int(data.worldPosition.x + x, 0, data.worldPosition.z + z), data, false);
    ...
    private BiomeGeneratorSelection SelectBiomeGenerator(Vector3Int worldPosition, ChunkData data, bool useDomainWarping = true) {
        ...
        List<BiomeSelectionHelper> biomeSelectionHelper = GetBiomeGeneratorSelectionHelper(worldPosition);
        BiomeGenerator generator_1 = SelectBiome(biomeSelectionHelper[0].Index);
        BiomeGenerator generator_2 = SelectBiome(biomeSelectionHelper[0].Index);

        float distance = Vector3.Distance(
            biomeCenters[biomeSelectionHelper[0].Index],
            biomeCenters[biomeSelectionHelper[1].Index]);

        float weight_0 = biomeSelectionHelper[0].Distance / distance;
    ...
    private BiomeGenerator SelectBiome(int index) {
        ...
        foreach (var data in biomeGeneratorsData) {
            if (temp >= data.temperatureStartThreshold && temp < data.tempaeratureEndThreshold) {
    ...
    private List<BiomeSelectionHelper> GetBiomeGeneratorSelectionHelper(Vector3Int position) {
        position.y = 0;
...
public struct BiomeData {
    [Range(0f, 1f)]
    public float temperatureStartThreshold, tempaeratureEndThreshold;
...
public class BiomeGeneratorSelection {
    ...
    public BiomeGeneratorSelection(BiomeGenerator biomeGenerator, int? terrainSurfaceNoise) {
