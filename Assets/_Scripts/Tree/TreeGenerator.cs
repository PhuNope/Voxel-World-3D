...
public class TreeGenerator : MonoBehaviour {
    ...
    public TreeData GenerateTreeData(ChunkData chunkData, Vector2Int mapSeedOffet) {
        treeNoiseSettings.worldOffset = mapSeedOffet;
        TreeData treeData = new TreeData();
        float[,] noiseData = GenerateTreeNoise(chunkData, treeNoiseSettings);
        treeData.treePositions = DataProcessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.y);
        return treeData;
    }
    ...
    private float[,] GenerateTreeNoise(ChunkData chunkData, NoiseSettings treeNoiseSettings) {
        ...
        int zMin = chunkData.worldPosition.x;
